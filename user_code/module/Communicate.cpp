...
void Communicate::run()
{
    ...
    //向底盘发送遥控器和云台数据
    int16_t temp_ch1, temp_ch2, temp_ch3;
    uint16_t temp_v;
    uint8_t temp_s1, temp_gimbal_behaviour;
    ...
    temp_gimbal_behaviour = gimbal.gimbal_behaviour;
    if (temp_gimbal_behaviour == GIMBAL_ABSOLUTE_ANGLE)
        temp_gimbal_yaw_angle = gimbal.gimbal_yaw_motor.absolute_angle;
    else
        temp_gimbal_yaw_angle = gimbal.gimbal_yaw_motor.relative_angle;

    can_receive.send_rc_board_com(temp_ch1, temp_ch2, temp_ch3, temp_v);
    can_receive.send_gimbal_board_com(temp_s1, temp_gimbal_behaviour, temp_gimbal_yaw_angle);
...
//                 can_receive.get_motor_measure(4, rx_data);
//                 detect_hook(GIMBAL_YAW_MOTOR_TOE);
//                 break;

//             case CAN_PITCH_MOTOR_ID:
//                 can_receive.get_motor_measure(5, rx_data);
